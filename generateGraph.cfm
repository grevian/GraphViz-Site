...
<!--- The only other concern here, is a malicious DOT script, which could potentially expose files on the system by including them
 as custom shapes for example, Since DOT has no way to restrict language features on the command line, make sure this app is sandboxed,
 your JSP servers security policies and system permissions are the only way to mitigate this danger. !--->
